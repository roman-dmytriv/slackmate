...
import os
from flask import Blueprint, redirect, request, jsonify, make_response, session, url_for
...
def slack_auth_callback():
    try:
        ...
        if state is None or state != request.args.get('state'):
            return make_response(jsonify({"error": "Invalid state parameter."}), 400)
        ...
        if auth_result is None:
            return make_response(jsonify({"error": "Authentication failed: No response from Slack API."}), 500)
        ...
        if isinstance(auth_result, SlackResponse) and auth_result["ok"]:
            ...
            return make_response(jsonify({"message": "Authentication successful! You can now use the Slack API."}), 200)

        elif isinstance(auth_result, dict) and 'error' in auth_result:
            return make_response(jsonify({"error": f"Authentication failed: {auth_result['error']}"}), 500)

        else:
            return make_response(jsonify({"error": "Authentication failed: Unexpected response from Slack API."}), 500)

    except SlackApiError as e:
        # Handle Slack API errors
        error_message = e.response['error']
        return make_response(jsonify({"error": f"Slack API Error: {error_message}"}), 500)
