...
def test_slack_auth_callback_successful_auth(client, monkeypatch):
    # Define a mock response from the Slack API for successful authentication
    mock_auth_result = {
        "ok": True,
        "team": {"id": "team_id"},
    ...
    monkeypatch.setattr(
        'app.auth.oauth.slack_client.oauth_v2_access',
    ...
    # Verify that the authentication was successful message is present in the response
